...
public class MUD
{
    ...
    private Map<String,Player> users = new HashMap<String,Player>();
    ...
    public String Items(String loc){
        return getVertex(loc).ItemsToString();
    }
    ...
    public void createEnemy(String enemy, int health, int attack){
        Enemy e = new Enemy();
        e.name = enemy;
        e.health = health;
        e.damage = attack;
    ...
    public String Inventory(){
        ...
        for(int i = 0; i<player.inventory.size(); i++){
            items = items + player.inventory.get(i);
            items += "\n";
        }
    ...
    public boolean collect(String item, String location){
        ...
        List<String> items = vertex._things;
        if(items.contains(item)){
            player.inventory.add(item);
            delThing(location,item);
            changeMessage(location, item + "item already collected");
            return true;
        }
        return false;
    }
    ...
    public Integer getPlayerHealth(String name){
        Player p = users.get(name);
        return p.health;
    }

    public String Attack(String user, String enemy){
        Player p = users.get(user);
        Enemy e = enemies.get(enemy);
        String log = "";
        log += "Attacking: " + enemy + "\n";
        if((e.health -= p.attack) < 0){
            e.health = 0;
        }

        log += "Dealt " + p.attack +" damage" + "\n";
        if(e.health>0){
            log += "Enemy has " + e.health + " health remaining" + "\n";
            log += "Enemy attack back!" + "\n";
            p.health -= e.damage;
        }
        if(p.health >= 0){
            log += "You have " + p.health + " Health remaining" + "\n";
        }
        if(e.health == 0){
            log += "You killed " + e.name +"\n";
        }
        return log;
    }
